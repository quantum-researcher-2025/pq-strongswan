...
/**
 * Check whether to send a RELIABLE_IKE_TRANSPORT notify
 */
static bool send_reliable_ike_transport(private_ike_init_t *this) {
    bool reliable_ike_transport = FALSE;
    if (this->initiator) {
        reliable_ike_transport = TRUE;
    } else if (this->ike_sa->supports_extension(this->ike_sa, EXT_RELIABLE_IKE_TRANSPORT)) {
        reliable_ike_transport = TRUE;
    }
    return reliable_ike_transport;
}
...
static bool build_payloads(private_ike_init_t *this, message_t *message)
{
    ...
    /* notify the peer if we want to use/support RELIABLE_IKE_TRANSPORT */
    if (!this->old_sa && send_reliable_ike_transport(this)){
        message->add_notify(message, FALSE, RELIABLE_IKE_TRANSPORT, chunk_empty);
    }
	return TRUE;
}
			...
			case PLV2_NOTIFY:
			{
				...
				switch (notify->get_notify_type(notify))
				{
					...
					case INTERMEDIATE_EXCHANGE_SUPPORTED:
						...
						break;
                    case RELIABLE_IKE_TRANSPORT:
                        if (!this->old_sa) {
                            this->ike_sa->enable_extension(this->ike_sa, EXT_RELIABLE_IKE_TRANSPORT);
                        }
                        break;
					default:
